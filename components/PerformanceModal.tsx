...
const RocCurveChart: React.FC<{ auc: number }> = ({ auc }) => {
  ...
  // A static, illustrative curve for a high-performing model
...
const performanceData = [
    { title: "ECG Analysis (Arrhythmia Detection)", auc: 0.97, f1: 0.94 },
    { title: "Radiology (Pneumonia Detection)", auc: 0.98, f1: 0.95 },
    { title: "Retina Analysis (Diabetic Retinopathy)", auc: 0.96, f1: 0.93 },
    { title: "Dermatoscopy (Melanoma Classification)", auc: 0.95, f1: 0.92 },
];
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">
              Illustrative AI Model Performance
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            <p className="text-sm text-gray-600 dark:text-gray-300">
                The metrics below are <strong>illustrative examples</strong> from standard academic datasets to provide a general sense of the AI's capabilities. They are not real-time calculations for your uploaded image but reflect the model's benchmarked performance on specific classification tasks.
            </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 items-center">
                            ...
                            <div className="flex flex-col justify-center">
                                <PerformanceMetric 
                                    label="AUC-ROC" 
                                    value={`~${data.auc}`}
                                    description="Measures the model's ability to distinguish between classes. A value of 1.0 represents a perfect model."
                                />
                                <PerformanceMetric 
                                    label="F1-Score" 
                                    value={`~${data.f1}`}
                                    description="A balance between precision and recall, indicating the model's accuracy. A value of 1.0 is perfect."
                                />
                            </div>
                        </div>
                    </div>
            ...
            <div className="mt-4 text-xs text-center text-gray-500 dark:text-gray-400 p-3 bg-amber-50 dark:bg-yellow-900/20 border border-amber-200 dark:border-yellow-800 rounded-lg">
                <strong>Important:</strong> Performance can vary based on image quality and the specific clinical case. This information is for educational purposes only and does not constitute a guarantee of accuracy. ALWAYS consult a qualified healthcare professional.
            </div>
        </div>
      </div>
    </div>
